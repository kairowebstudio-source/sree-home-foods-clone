...
export const addProduct = createServerFn({ method: "POST" }).validator((d: ProductInput) => d).handler(async ({ data }) => { await requireAdmin(); if (!supabaseEnabled()) throw new Error(NEEDS_ENV_MSG); const client = supabaseAdmin(); if (data.image?.startsWith("data:") && data.image.length > 1_500_000) throw new Error("Failed to add product: image is too large. Re-upload with a smaller file."); const product = { slug: data.slug, name: data.name, tagline: data.tagline, category: data.category, weight: data.weight, price: data.price, mrp: data.mrp || null, variants: data.variants ?? [], image: data.image || "/placeholder.svg", description: data.description, benefits: data.benefits, delivery_charge: (data as any).delivery_charge ?? null, stock: (data as any).stock ?? null }; const { error } = await client.from("products").insert(product); if (error) throw new Error(withSchemaHint("Failed to add product", error)); return product as Product; });

export const updateProduct = createServerFn({ method: "POST" }).validator((d: Product) => d).handler(async ({ data }) => { await requireAdmin(); if (!supabaseEnabled()) throw new Error(NEEDS_ENV_MSG); const client = supabaseAdmin(); const incomingImage = (data.image ?? "").trim(); let imagePatch: string | undefined; if (incomingImage && !incomingImage.startsWith("data:")) imagePatch = incomingImage; else if (incomingImage.startsWith("data:")) { const { data: existing } = await client.from("products").select("image").eq("slug", data.slug).maybeSingle(); const stored = (existing?.image as string | null) ?? ""; if (stored !== incomingImage) { if (incomingImage.length > 1_500_000) throw new Error("Failed to update product: image is too large. Re-upload the image with a smaller file."); imagePatch = incomingImage; } } const { error } = await client.from("products").update({ name: data.name, tagline: data.tagline, category: data.category, weight: data.weight, price: data.price, mrp: data.mrp || null, variants: data.variants ?? [], ...(imagePatch !== undefined ? { image: imagePatch } : {}), description: data.description, benefits: data.benefits, delivery_charge: (data as any).delivery_charge ?? null, stock: (data as any).stock ?? null, updated_at: new Date().toISOString() }).eq("slug", data.slug); if (error) throw new Error(withSchemaHint("Failed to update product", error)); return data; });
