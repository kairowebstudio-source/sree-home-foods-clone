...
// ── Server Functions ───────────────────────────────────────────
// Admin login keeps the original simple password fallback for this project.
// ADMIN_PASSWORD and ADMIN_SESSION_SECRET can still override these defaults.
...
function adminSessionConfig() {
  const password = process.env.ADMIN_SESSION_SECRET || DEFAULT_ADMIN_SESSION_SECRET;
  return {
    password,
...
function withSchemaHint(action: string, error: { message?: string }): string {
  if (error?.message && error.message.includes("in the schema cache")) return `${action}: ${error.message} — run the migrations in supabase/migrations/ in your Supabase project.`;
  return `${action}: ${error?.message ?? "unknown error"}`;
}

export const addProduct = createServerFn({ method: "POST" }).validator((d: ProductInput) => d).handler(async ({ data }) => {
  await requireAdmin();
  if (!supabaseEnabled()) throw new Error(NEEDS_ENV_MSG);
  const client = supabaseAdmin();
  if (data.image?.startsWith("data:") && data.image.length > 1_500_000) throw new Error("Failed to add product: image is too large. Re-upload with a smaller file.");
  const product = { slug: data.slug, name: data.name, tagline: data.tagline, category: data.category, weight: data.weight, price: data.price, mrp: data.mrp || null, variants: data.variants ?? [], image: data.image || "/placeholder.svg", description: data.description, benefits: data.benefits };
...
export const updateProduct = createServerFn({ method: "POST" }).validator((d: Product) => d).handler(async ({ data }) => {
  await requireAdmin();
  if (!supabaseEnabled()) throw new Error(NEEDS_ENV_MSG);
  const client = supabaseAdmin();
  const incomingImage = (data.image ?? "").trim();
  let imagePatch: string | undefined;
  if (incomingImage && !incomingImage.startsWith("data:")) imagePatch = incomingImage;
  else if (incomingImage.startsWith("data:")) {
    const { data: existing } = await client.from("products").select("image").eq("slug", data.slug).maybeSingle();
    const stored = (existing?.image as string | null) ?? "";
    if (stored !== incomingImage) {
      if (incomingImage.length > 1_500_000) throw new Error("Failed to update product: image is too large. Re-upload the image with a smaller file.");
      imagePatch = incomingImage;
    }
  }
  const { error } = await client.from("products").update({ name: data.name, tagline: data.tagline, category: data.category, weight: data.weight, price: data.price, mrp: data.mrp || null, variants: data.variants ?? [], ...(imagePatch !== undefined ? { image: imagePatch } : {}), description: data.description, benefits: data.benefits, updated_at: new Date().toISOString() }).eq("slug", data.slug);
  if (error) throw new Error(withSchemaHint("Failed to update product", error));
  return data;
});

export const deleteProduct = createServerFn({ method: "POST" }).validator((d: string) => d).handler(async ({ data }) => {
  await requireAdmin();
  if (!supabaseEnabled()) throw new Error(NEEDS_ENV_MSG);
  const client = supabaseAdmin();
...
async function calculateAuthoritativeOrder(data: OrderData) {
  ...
  const { data: productsRows, error } = await client.from("products").select("slug,name,price,variants").in("slug", slugs);
  ...
  const items = data.items.map((item) => {
    ...
    if (item.weight && variants.length) {
      ...
      if (!variant || !Number.isFinite(Number(variant.price))) throw new Error(`The selected ${item.weight} variant for ${product.name} is unavailable.`);
...
export const submitOrder = createServerFn({ method: "POST" }).validator((d: OrderData) => d).handler(async ({ data }) => {
  ...
  const { data: order, error } = await client.from("orders").insert({ customer_name: data.customer_name.trim(), phone: data.phone.replace(/\D/g, ""), email: data.email.trim().toLowerCase(), address: fullAddress, city: data.city.trim(), pincode: data.pincode, items: calculated.items, total: calculated.total, notes: data.notes.trim(), status: "pending", payment_method: data.method, payment_status: "pending" }).select("id").single();
  if (error) throw new Error(`Failed to save order: ${error.message}`);
  if (data.method === "cod" && data.notify !== false) {
    try { await sendOrderConfirmationEmail({ orderId: order.id, customerName: data.customer_name, email: data.email, phone: data.phone, address: fullAddress, notes: data.notes, items: calculated.items, total: calculated.total, method: "cod" }); } catch (err) { console.error("Failed to send order confirmation email:", err); }
    try { await sendOwnerOrderNotificationEmail({ orderId: order.id, customerName: data.customer_name, email: data.email, phone: data.phone, address: fullAddress, items: calculated.items, total: calculated.total, method: "cod", paymentStatus: "pending" }); } catch (err) { console.error("Failed to send owner order notification:", err); }
  }
  return { orderId: order.id, total: calculated.total };
});

export async function sendOrderConfirmationEmail(input: { orderId: string; customerName: string; email: string; phone: string; address: string; notes: string; items: { name: string; price: number; qty: number }[]; total: number; method: "cod" | "online" }): Promise<void> {
...
export async function sendOwnerOrderNotificationEmail(input: { orderId: string; customerName: string; email: string; phone: string; address: string; items: { name: string; price: number; qty: number }[]; total: number; method: "cod" | "online"; paymentStatus: string }): Promise<void> {
  ...
  const resend = new Resend(apiKey);
  const from = process.env.EMAIL_FROM || "Sree Home Foods <onboarding@resend.dev>";
  const { error } = await resend.emails.send({ from, to: ownerEmail, subject: `New Sree Home Foods Order — ${input.orderId.slice(0, 8).toUpperCase()}`, html });
  if (error) throw new Error(error.message);
}
...
export const uploadProductImage = createServerFn({ method: "POST" }).validator((d: { base64: string; filename: string; contentType: string }) => d).handler(async ({ data }) => {
  await requireAdmin();
...
export const addCategory = createServerFn({ method: "POST" }).validator((d: string) => d).handler(async ({ data }) => {
  await requireAdmin();
  if (!supabaseEnabled()) throw new Error(NEEDS_ENV_MSG);
...
export const deleteCategory = createServerFn({ method: "POST" }).validator((d: string) => d).handler(async ({ data }) => {
  await requireAdmin();
  if (!supabaseEnabled()) throw new Error(NEEDS_ENV_MSG);
  const client = supabaseAdmin();
